...
export const CartProvider = ({children}: {children: React.ReactNode}) => {
    ...
    const total = products.reduce((acc, product) => acc + product.price_in_cents * product.quantity, 0);
    ...
    const decreaseProductQuantity = (productId: string) => {
        setProducts((prevProducts) => {
            return prevProducts.map((prevProduct) => {
                if(prevProduct.id === productId && prevProduct.quantity > 1) return {...prevProduct, quantity: prevProduct.quantity - 1};
                return prevProduct;
            })
        })
    }
    ...
    const increaseProductQuantity = (productId: string) => {
        setProducts((prevProducts) => {
            return prevProducts.map((prevProduct) => {
                if(prevProduct.id === productId) return {...prevProduct, quantity: prevProduct.quantity + 1};
                return prevProduct;
            })
        })
    }
    ...
    return (
        <CartContext.Provider 
            value={{isOpen, products, total, toggleCart, addProduct, decreaseProductQuantity, increaseProductQuantity, removeProduct}}>
            {children}
        </CartContext.Provider>
    )
}
